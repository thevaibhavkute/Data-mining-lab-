...
def test(numericDf, window, plotMehthod):
    # Create the root window
    root = Tk()
    root.geometry('180x200')

    label1 = Label(root, text="Select 2 options")
    label1.pack()
    ...
    def selected_item():
        ...
        for i in listbox.curselection():
            print(col.append(listbox.get(i)))

        print(col)
        scatterPlot(numericDf, col[0], col[1])
...
def selectTwoAttribute():
    print("Inside two attribute")
...
def graphicalAnalysis(numericDf):
    ...
    quantile = Button(menu, text="Quantile - Quantile",
                      command=partial(test, numericDf, window=menu, plotMehthod=qqPlot))
    quantile.grid(row=2, column=1)

    histogram = Button(menu, text="Histogram",
                       command=partial(selectOneAttribute, numericDf, plotMehthod=histogramPlot))
    histogram.grid(row=3, column=1)

    scatter = Button(menu, text="Scatter", command=partial(test, numericDf, window=menu, plotMehthod=scatterPlot))
...
def typeOfAnalysis(filePath):
    filePath = r"D:\abhishek\7th SEM\DM Lab\Assignment 2\Iris.csv"
