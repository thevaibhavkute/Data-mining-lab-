...
def displayRules():
	global clf
	data = tree.export_text(clf)
	new_df = pd.read_csv(io.StringIO(data), sep='/s+', engine = 'python')
...
def appWindow():
	...
	Decision_Tree.add_command(label = 'Display rules', command = displayRules)
	...
	Performance_Evaluation.add_command(label = 'Performance Of Rules', command = performance_of_rules)
